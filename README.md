...
<img width="2321" height="1524" alt="Dugoga-architecture" src="https://github.com/user-attachments/assets/b55ec8d4-e0e7-44d7-b519-88d452bdc2da" />
</details>
...
-  Java 17 플랫폼 스레드의 한계 
   - 운영체제(OS) 스레드와 1:1로 매핑되는 기존 모델은 스레드 증가에 따른 메모리 소모와 컨텍스트 스위칭 비용이 급격히 상승하는 구조적 한계가 있습니다. 
- OS 스레드가 아닌 JVM 내부 경량 구조로 관리되는 가상 스레드를 활용하여, 컨텍스트 스위칭 비용을 최소화하고 대량의 동시 요청을 안정적으로 처리합니다.
- 전국 단위 서비스 확장을 고려하여 대용량 메모리 환경에서 낮은 지연 시간을 보장하는 Java 21의 개선된 ZGC 활용 가능성을 열어두었습니다. (현재는 G1 GC를 기본으로 사용 중입니다.)

### Spring boot 3.5.11

- **가상 스레드 완전 지원**: Java 21의 핵심인 가상 스레드를 프레임워크 차원에서 완벽히 지원하여 인프라 효율을 최대로 끌어올립니다.
- **안정성**: 풍부한 레퍼런스를 바탕으로 개발 안정성을 확보했으며, 향후 Spring Boot 4.0으로의 원활한 마이그레이션이 용이한 최신 버전을 채택했습니다.
...
- 관계형 데이터베이스(RDB) 선정 이유 
   - **인덱스 최적화**: B-Tree 구조의 특성을 활용해 정렬 및 범위 검색(Range Scan)에서 뛰어난 성능을 발휘합니다. 
   - **복잡한 관계 처리**: 배달 시스템 특성상 복잡한 Join 연산을 효율적으로 관리할 수 있습니다. 
   - **엄격한 데이터 관리**: NoSQL 대비 트랜잭션 관리가 용이하며, 배달 서비스에서 가장 중요한 데이터 신뢰도와 ACID 준수 능력이 탁월합니다.

### Redis
- **실시간 보안 및 성능 확보**: 로그아웃한 사용자의 AccessToken을 블랙리스트로 관리합니다. 모든 요청의 보안 필터에서 발생하는 잦은 조회 작업을 인메모리 DB의 빠른 성능으로 처리합니다. 
- **지리 정보(Geospatial) 자료구조**: Redis의 지리 데이터 처리 기능을 미리 반영했습니다. 이는 향후 서비스 전국 확대 시, 가게 거리별 정렬 및 위치 기반 검색 로직을 유연하게 구현하기위해 선택했습니다.
